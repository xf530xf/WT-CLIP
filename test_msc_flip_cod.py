...
parser.add_argument("--config",
                    default='/data2/XF/KeYan/WeCLIP/configs/cod_attn_reg.yaml',
                    type=str,
                    help="config")
parser.add_argument("--work_dir", default="/data2/XF/KeYan/WeCLIP/results-cod", type=str, help="work_dir")
...
parser.add_argument("--model_path", default="/data2/XF/KeYan/WeCLIP/work_dir_voc/checkpoints/2024-12-20-00-45/WeCLIP_model_iter_28800.pth", type=str, help="model_path")
...
def validate(model, dataset, test_scales=None):
    ...
    _preds_hist = np.zeros((65, 65))
    _msc_preds_hist = np.zeros((65, 65))
    _cams_hist = np.zeros((65, 65))
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
        ...
        resized_segs = F.interpolate(segs, size=cam_cat.shape[1:], mode='bilinear', align_corners=False)
        seg_preds = torch.argmax(resized_segs, dim=1)

        resized_msc_segs = F.interpolate(msc_segs, size=cam_cat.shape[1:], mode='bilinear', align_corners=False)
        ...
        _gts += list(labels.cpu().numpy().astype(np.int16))
               ...
               # 创建 cam 结果文件夹
        os.makedirs(args.work_dir + "/cam_images", exist_ok=True)

        # 处理每个 cam 并保存
        for i, c in enumerate(cam.cpu().numpy()):
            c = (c - c.min()) / (c.max() - c.min()) * 255  # 归一化到 0-255
            c = c.astype(np.uint8)  # 转换为 uint8
            imageio.imsave(os.path.join(args.work_dir, "cam_images", f"{name[0]}_{i}.png"), c)
        ...
        np.save(args.work_dir+ '/logit_cod/' + name[0] + '.npy', {"segs":segs.detach().cpu().numpy(), "msc_segs":msc_segs.detach().cpu().numpy()})
...
def crf_proc(config):
    ...
    def _job(i):
        ...
        name = name_list[i]
        logit_name = os.path.join(args.work_dir, "logit_cod", name + ".npy")
        ...
        imageio.imsave(os.path.join(args.work_dir, "prediction_cod", name + ".png"), np.squeeze(pred).astype(np.uint8))
        imageio.imsave(os.path.join(args.work_dir, "prediction_cmap_cod", name + ".png"), encode_cmap(np.squeeze(pred)).astype(np.uint8))
        return pred, label
    ...
    # hist = np.zeros((65,65))
    # hist, score = evaluate.scores(gts, preds, hist, 65)
...
def main(cfg):
    ...
    val_dataset = cod.VOC12SegDataset(
...
if __name__ == "__main__":
    ...
    os.makedirs(args.work_dir + "/logit_cod", exist_ok=True)
    os.makedirs(args.work_dir + "/prediction_cod", exist_ok=True)
    os.makedirs(args.work_dir + "/prediction_cmap_cod", exist_ok=True)
