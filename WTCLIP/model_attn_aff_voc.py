...
from WTCLIP.PAR import PAR
# from WTCLIP.DEM import MEEM, MLFusion, AdaptiveFeatureFusion
import random
...
#########################################################
# 为背景类生成模板
def generate_bg_template():
    # 背景类描述模板，模板中的 `{}` 会作为占位符
    templates = [
        # "A wide view of {}.",
        # "A clear image of {} in the environment.",
        # "A scenic view of {}."
        "a clean origami {}."
    ]
    # 随机选择一个模板并返回
...
# 为前景类生成模板
def generate_fg_template():
    # 前景类描述模板，模板中的 `{}` 会作为占位符
    templates = [
        # "A {} on display.",
        # "A {} placed in a scene.",
        # "A {} positioned in a space."
        "a clean origami {}."
    ]
    # 随机选择一个模板并返回
    return random.choice(templates)
...
def zeroshot_classifier(classnames, templates, model):
    ...
    with torch.no_grad():
        ...
        for classname in classnames:
            ...
            class_embeddings = model.encode_text(texts) #embed with  clip text encoder
...
def zeroshot_classifier_FG(classnames,model):
    

    with torch.no_grad():
        zeroshot_weights = []
        for classname in classnames:
            templates=[generate_fg_template()]
            texts = [template.format(classname) for template in templates] #format with class
            # print("@@@@@@@@@@@@@@@@@@@",texts)
            texts = clip.tokenize(texts).cuda() #tokenize
            class_embeddings = model.encode_text(texts) #embed with  clip text encoder
...
def zeroshot_classifier_BG(classnames,model):
    with torch.no_grad():
        zeroshot_weights = []
        for classname in classnames:
            templates=[generate_bg_template()]
            texts = [template.format(classname) for template in templates] #format with class
            print("@@@@@@@@@@@@@@@@@@@",texts)
            texts = clip.tokenize(texts).cuda() #tokenize
            class_embeddings = model.encode_text(texts) #embed with  clip text encoder
...
class WTCLIP(nn.Module):
    def __init__(self, num_classes=None, clip_model=None, embedding_dim=256, in_channels=512, dataset_root_path=None, device='cuda'):
        ...
        # self.bg_text_features = zeroshot_classifier_BG(BACKGROUND_CATEGORY,  self.encoder)
        # self.fg_text_features = zeroshot_classifier_FG(new_class_names, self.encoder)
    ...
    def forward(self, img, img_names='2007_000032', mode='train'):
        ...
        fts_cam=fts_all.copy()

        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!",fts_all[0].shape)

        # fts_all_stack=self.WD(fts_all)
