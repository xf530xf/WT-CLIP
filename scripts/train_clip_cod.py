...
from WTCLIP.model_attn_aff_cod import WTCLIP
...
def validate(model=None, data_loader=None, cfg=None):
    ...
    num = 1
    seg_hist = np.zeros((65, 65))
    cam_hist = np.zeros((65, 65))
    ...
    for _, data in tqdm(enumerate(data_loader),
        ...
        # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@",segs.shape,labels.shape)
        resized_segs = F.interpolate(segs, size=cam.shape[1:], mode='bilinear', align_corners=False)
...
def train(cfg):
    ...
    train_dataset = cod.VOC12ClsDataset(
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        if (n_iter + 1) % cfg.train.log_iters == 0:
            ...
            seg_hist, seg_score = evaluate.scores(gts, preds, seg_hist)
            # cam_hist, cam_score = evaluate.scores(gts, fts_cam, cam_hist)
