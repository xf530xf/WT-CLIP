...
from WTCLIP_model.model_attn_aff_coco import WTCLIP
...
parser.add_argument("--config",
                    ...
                    type=str,
                    help="config")
parser.add_argument("--work_dir", default="/data2/XF/KeYan/WTCLIP/results", type=str, help="work_dir")
...
parser.add_argument("--model_path", default="/data2/XF/KeYan/WTCLIP/work_dir_coco/checkpoints/2025-01-05-19-26/WTCLIP_model_iter_80000.pth",
...
def validate(model, dataset, test_scales=None):
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
        ...
        np.save(args.work_dir+ '/logit_coco/' + name[0] + '.npy', {"segs":segs.detach().cpu().numpy(), "msc_segs":msc_segs.detach().cpu().numpy()})
...
def crf_proc(config):
    ...
    def _job(i):
        ...
        name = name_list[i]
        logit_name = os.path.join(args.work_dir, "logit_coco", name[13:] + ".npy")
        ...
        imageio.imsave(os.path.join(args.work_dir, "prediction_coco", name + ".png"), np.squeeze(pred).astype(np.uint8))
        imageio.imsave(os.path.join(args.work_dir, "prediction_cmap_coco", name + ".png"), encode_cmap(np.squeeze(pred)).astype(np.uint8))
        return pred, label
    ...
    hist = np.zeros((81, 81))
    score = evaluate.scores(gts, preds,hist,num_classes=81)
...
if __name__ == "__main__":
    ...
    os.makedirs(args.work_dir + "/logit_coco", exist_ok=True)
    os.makedirs(args.work_dir + "/prediction_coco", exist_ok=True)
    os.makedirs(args.work_dir + "/prediction_cmap_coco", exist_ok=True)
